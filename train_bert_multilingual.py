...
def build(args):
    ...
    tokenizer = BertTokenizer.from_pretrained(PRETRAINED_MODEL_NAME, tokenize_chinese_chars=False)
...
def train(args, model, trainloader, devloader, testloader, tokenizer):
    ...
    max_score = 0
    iters = 0
#     writer = SummaryWriter(f'{args.plot_dir}/bert-base-multilingual_lr_{args.lr}_pre{args.load_pretrained_model}')
...
def main():
    ...
    parser.add_argument('--pretrained_model_path', type=str, default='save/QA_1e-05.pkl', help='pretrain model name')
    ...
    args.model_dir = 'save_0627'
    if not os.path.isdir(args.model_dir):
        os.makedirs(args.model_dir)

    args.log_dir = 'train_log_0627'
    if not os.path.isdir(args.log_dir):
        os.makedirs(args.log_dir)

    args.output_dir = 'output_0627'
...
#         os.makedirs(args.plot_dir)
    ...
    logger.info(args)
#     logger.info('bert-base-multilingual')
